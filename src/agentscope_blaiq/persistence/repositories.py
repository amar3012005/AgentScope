...
class BrandDnaRepository:
    ...
    async def create_job(self, job_id: str, tenant_id: str, upload_ids: list[str]) -> BrandDnaExtractionJobRecord:
        record = BrandDnaExtractionJobRecord(
            job_id=job_id,
            tenant_id=tenant_id,
    ...
    async def get_job(self, job_id: str) -> BrandDnaExtractionJobRecord | None:
        return await self.session.get(BrandDnaExtractionJobRecord, job_id)
    ...
    async def update_job(
        ...
        intermediate_json: str | None = None,
        ...
        if intermediate_json:
            job.intermediate_json = intermediate_json
        if result_json:
...
class WorkflowRepository:
    ...
    def _dump_state_payload(self, workflow: WorkflowRecord, payload: dict[str, Any]) -> str:
        ...
        payload.setdefault("analysis_mode", "standard")
    ...
    async def update_workflow_snapshot(
        ...
        *,
        ...
        if workflow_mode is not None:
            ...
            state_payload["workflow_mode"] = workflow_mode.value
        if analysis_mode is not None:
            state_payload["analysis_mode"] = analysis_mode.value
        ...
        state_payload.update(
            {
                ...
                "status": workflow.status,
                "workflow_mode": workflow.workflow_mode,
                "analysis_mode": state_payload.get("analysis_mode", "standard"),
                "current_node": workflow.current_node,
                "current_phase": workflow.current_phase,
                "current_agent": workflow.current_agent,
                "latest_event": workflow.latest_event,
    ...
    async def prepare_resume(self, request: ResumeWorkflowRequest) -> SubmitWorkflowRequest:
        ...
        snapshot = await self.get_status(request.thread_id)
        pending_node = snapshot.pending_node if snapshot and snapshot.pending_node else "hitl_evidence"
        resume_cursor = snapshot.resume_cursor if snapshot and snapshot.resume_cursor else pending_node
        await self.update_workflow_snapshot(
            ...
            pending_node=pending_node,
            resume_cursor=resume_cursor,
            last_completed_node=snapshot.last_completed_node if snapshot else None,
        )
