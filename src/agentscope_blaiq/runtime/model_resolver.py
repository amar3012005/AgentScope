    ...
    def resolve(self, role: str) -> ResolvedModel:
        ...
        if role_key == "routing":
            return self._build_resolved_model(
                role=role_key,
                model_name=self.settings.routing_model,
                temperature=0.0,
        ...
        if role_key == "text_buddy":
            resolved = self._build_resolved_model(
                role=role_key,
                model_name=self.settings.text_buddy_model,
                temperature=self.settings.text_buddy_temperature,
                fallback_model=self.settings.llm_fallback_model,
            )
            return ResolvedModel(
                role=resolved.role,
                ...
                api_base=resolved.api_base,
                timeout_seconds=resolved.timeout_seconds,
                max_output_tokens=self.settings.text_buddy_max_output_tokens,
                temperature=resolved.temperature,
                reasoning_effort=resolved.reasoning_effort,
                fallback_model=resolved.fallback_model,
            )
        ...
        *,
        ...
        try:
            ...
            if resolved.fallback_model and resolved.fallback_model != resolved.model_name:
                ...
                kwargs["model"] = fallback.model_name if fallback.api_base else self._strip_provider_prefix(fallback.model_name)
