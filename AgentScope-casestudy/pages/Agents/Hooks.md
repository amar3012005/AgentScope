...
Supported hook types in AgentScope
Agent Class
...
Since hooks in AgentScope are implemented using a metaclass, they support inheritance.
...
The signature of all pre-hooks
Name
...
All positional arguments and keyword arguments of the core function are passed as a single kwargs dict to the hook functions
...
For post hooks, an additional output argument is added to the signature, which represents the output of the target function. If the core function has no output, the output argument will be None.

The signature of all post-hooks
Name
...
def post_hook_template(
    ...
    pass
Hook Management
AgentScope provides both instance- and class-level hooks, depending on the effective scope of the hooks. They execute in the following order:

Hooks in AgentScope
AgentScope provides built-in methods to manage hooks at both instance and class levels as follows:

Hook management methods in AgentScope
Level
...
Download Jupyter notebook: task_hook.ipynb
