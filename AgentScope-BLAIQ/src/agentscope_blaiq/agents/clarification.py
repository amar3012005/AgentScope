...
The research agent found {evidence_context['source_count']} sources. Here are the top findings:
{findings_text}

The artifact type is: {artifact_family.value.replace('_', ' ')}

Generate 2-4 clarification questions that are SPECIFIC to this request and these findings.

RULES:
1. Questions must reference SPECIFIC details from the findings (company names, product names, technical terms, metrics).
2. answer_options must be DERIVED FROM THE EVIDENCE — use actual topics, products, or details found in the research.
3. NEVER use generic options like "Use best judgement", "Keep it concise", "Standard".
4. Each option should be a concrete choice the user can select, based on what was found.
5. The headline should reference what was found (e.g. "I found details about SOLVIS solar systems — help me focus")
6. Keep questions SHORT and actionable — the user should be able to answer in 1 click.

Example of GOOD questions (when evidence mentions "SolvisMax" and "SolvisPia"):
- "Which product line should be the focus?" with options ["SolvisMax hybrid system", "SolvisPia compact heater", "Full product portfolio", "Something else"]
- "What's the target audience for this?" with options ["Homeowners upgrading heating", "Commercial building managers", "Solar installers/contractors"]

Example of BAD questions:
- "What angle should we lead with?" with options ["Use best judgement", "Keep it concise"] — TOO GENERIC

Return ONLY valid JSON:
{{
  "headline": "specific headline referencing what was found",
  "intro": "1-2 sentence intro mentioning key findings",
  "questions": [
    {{
      "requirement_id": "field:target_audience",
      "question": "specific question using evidence details",
      "why_it_matters": "why this matters for the output",
      "answer_hint": "what kind of answer is expected",
      "answer_options": ["evidence-based option 1", "evidence-based option 2", "evidence-based option 3", "I'll provide my own"]
    }}
  ]
}}"""
        ...
        # Ensure every question has options — but prefer LLM-generated ones
        for question in final_questions:
            if not question.answer_options:
                question.answer_options = ["Use the research findings", "I'll provide specific direction", "Skip this question"]
