...
_TEXT_FAMILIES = ["email", "invoice", "letter", "memo", "proposal", "report", "social_post", "summary", "direct"]
...
class TextBuddyAgent(BaseAgent):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.status_messages = [
            "Initializing brand-voice composition engine...",
            "Loading artifact templates and voice guidelines...",
            "Synthesizing research evidence into draft sections...",
        ]

    def build_toolkit(self) -> Toolkit:
        if self._shared_toolkit is not None:
            return self._shared_toolkit
    ...
    async def _generate_completion_summary(
        ...
        *,
        ...
        """Generate a 2-4 sentence natural language summary of the completed artifact.

        Shown in chat instead of the full artifact content.
        """
        citation_pattern = re.compile(r"\[source:[^\]]+\]")
        citations = citation_pattern.findall(content)
        unique_sources = list(dict.fromkeys(citations))[:4]
        source_note = (
            f"Based on {len(unique_sources)} source(s): {', '.join(unique_sources)}."
            if unique_sources
            else "No external sources cited."
        )

        summary_prompt = (
            f"You just composed a {family_key.replace('_', ' ')} for the following request:\n"
            f"\"{user_query}\"\n\n"
            f"Evidence used: {source_note}\n\n"
            "Write a 2-4 sentence completion summary for the user that:\n"
            "- States what was created (artifact type and key topic/recipient)\n"
            "- Mentions the main points or purpose covered\n"
            "- Notes key sources cited (inline [source:ID] if relevant)\n"
            "- Does NOT reproduce the artifact content verbatim\n"
            "- Ends with: the artifact is ready in the preview panel\n"
            "Write in first person. Be concise and natural."
        )

        try:
            runtime_agent = self._create_runtime_agent()
            response = await runtime_agent.reply(
                self.make_msg(summary_prompt, role="user", phase="summary"),
            )
            summary = self._extract_msg_text(response).strip()
            if summary and len(summary) > 20:
                return summary
        except Exception as exc:
            logger.debug("completion_summary generation failed: %s", exc)

        # Deterministic fallback
        family_label = family_key.replace("_", " ")
        return (
            f"I've composed a {family_label} based on your request. "
            f"The {family_label} covers the key points from your query and is ready in the preview panel."
        )
    ...
    async def compose(
        ...
        *,
        ...
        """
        await self.log_user("Drafting content with your enterprise brand voice...")
        ...
        try:
            ...
            await self.log_user("Refining structure and tone...")
            response = await runtime_agent.reply(
                self.make_msg(
        ...
        await self.log_user("Checking citations and formatting final output...")
        await self.log(
        ...
        completion_summary = await self._generate_completion_summary(
            family_key=family_key,
            user_query=user_query,
            content=content,
