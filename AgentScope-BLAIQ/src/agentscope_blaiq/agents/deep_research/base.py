...
class BlaiqDeepResearchAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.status_messages = [
            "Scanning HIVE-MIND memory for enterprise context...",
            "Initializing deep research stack...",
            "Expanding search horizons across web and documents...",
        ]

    async def log_starting(self) -> None:
        """Log the first status message to indicate the research has started."""
        if hasattr(self, "status_messages") and self.status_messages:
            await self.log_user(self.status_messages[0])
        else:
            await self.log_user(f"Research agent is initializing...")
    ...
    async def log_user(
        ...
        *,
        ...
        """Emit a user-visible status update for the workflow stream."""
    ...
    async def gather(
        ...
        await self.log_user("Searching memory for relevant context...")
        ...
        await self.log_user(
            f"Found {len(phase1_findings)} relevant memories. Scanning sources...",
            detail={"memory_findings": len(phase1_findings)},
        )
        ...
        await self.log_user("Synthesizing evidence pack...")
        await self._log(
    ...
    async def _quick_recall_gather(
        ...
        await self.log_user(
            f"Found {len(findings)} relevant memories. Scanning sources...",
            detail={"memory_findings": len(findings)},
        )
        await self._log(
    ...
    async def _store_findings_in_hivemind(self, query: str, pack: EvidencePack) -> None:
        """Research write-back is disabled in the default run path."""
        del query, pack
        return
