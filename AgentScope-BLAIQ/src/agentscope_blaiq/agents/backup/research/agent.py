...
class ResearchAgent(BaseAgent):
    ...
    RECALL_LIMITS = {"quick": 5, "insight": 10, "panorama": 15}
    ...
    def __init__(self, *, hivemind: HivemindMCPClient | None = None, **kwargs) -> None:
        super().__init__(
            ...
            role="research",
            sys_prompt=(
                "You are the BLAIQ Research Specialist. Your primary responsibility is to gather "
                "evidence, normalize sources, and assign confidence to findings. \n\n"
                "STRICT BOUNDARIES:\n"
                "• HIVE-MIND memory is your primary ground truth.\n"
                "• Live web is for freshness and filling gaps only.\n"
                "• You do NOT write final artifact copy.\n"
                "• You do NOT design visual hierarchy.\n"
                "• You MUST assign confidence scores and provenance to every finding."
            ),
            **kwargs,
        )
    ...
    def build_toolkit(self) -> Toolkit:
        toolkit = Toolkit()
        self.register_tool(toolkit, tool_id="hivemind_recall", fn=self._tool_hivemind_recall, description="Recall enterprise memories relevant to the current request.")
        self.register_tool(toolkit, tool_id="hivemind_query_with_ai", fn=self._tool_hivemind_query_with_ai, description="Run synthesis over enterprise memory when direct recall is insufficient.")
        self.register_tool(toolkit, tool_id="hivemind_get_memory", fn=self._tool_hivemind_get_memory, description="Fetch a known memory record by id.")
        self.register_tool(toolkit, tool_id="hivemind_traverse_graph", fn=self._tool_hivemind_traverse_graph, description="Traverse related memories and linked decisions.")
        self.register_tool(toolkit, tool_id="hivemind_web_search", fn=self._tool_hivemind_web_search, description="Search the live web through HIVE-MIND when freshness is needed.")
        self.register_tool(toolkit, tool_id="hivemind_web_crawl", fn=self._tool_hivemind_web_crawl, description="Crawl URLs through HIVE-MIND for extraction.")
        self.register_tool(toolkit, tool_id="hivemind_web_job_status", fn=self._tool_hivemind_web_job_status, description="Resolve async HIVE-MIND web jobs.")
        self.register_tool(toolkit, tool_id="hivemind_web_usage", fn=self._tool_hivemind_web_usage, description="Inspect HIVE-MIND web quota and usage.")
        self.register_tool(toolkit, tool_id="validate_document_path", fn=self._tool_validate_document, description="Validate an uploaded document path before using it for research.")
        return toolkit
    ...
    async def _tool_hivemind_recall(self, query: str, limit: int = 5, mode: str = "quick"):
    ...
    def _recall_profile_for_query(cls, query: str) -> tuple[str, int]:
        ...
        return mode, cls.RECALL_LIMITS.get(mode, 5)
    ...
    async def _gather_memory(self, query: str) -> tuple[list[SourceRecord], list[EvidenceFinding], list[Citation], str | None]:
        ...
        # await self.log(
        #     f"Using HIVE-MIND recall mode: {recall_mode} (limit={recall_limit}).",
        #     kind="decision",
        # )
        for index, planned_query in enumerate(planned_queries, start=1):
            ...
            try:
                ...
                await self.log(
                    f"HIVE-MIND recall pass {index} failed: {exc}",
                    kind="decision",
                    detail={"query": planned_query, "mode": recall_mode},
                )
    ...
    async def answer_question(
        ...
        if "brief" in depth_lower:
            depth_guidance = (
                "Write 1-2 short paragraphs. Keep it crisp and executive. "
                "Do not over-explain. Include only the most important supported facts."
            )
            depth_style = "brief"
            depth_template = (
                "Template:\n"
                "1. Direct answer in one sentence.\n"
                "2. One short supporting sentence with the clearest evidence-backed detail.\n"
            )
        elif "technical" in depth_lower or "breakdown" in depth_lower:
            depth_guidance = (
                "Write 3-5 paragraphs. Give a richer, more context-heavy explanation. "
                "Group related product families, explain the structure of the portfolio, and mention representative examples. "
                "If the exact count is unknown, explain why and then describe the evidence-backed breadth in detail."
            )
            depth_style = "technical"
            depth_template = (
                "Template:\n"
                "1. Opening paragraph: direct answer and the core caveat, if any.\n"
                "2. Middle paragraphs: group the evidence into product families or categories.\n"
                "3. Final paragraph: explain what the evidence supports and what remains ambiguous.\n"
            )
        elif "detailed" in depth_lower:
            depth_guidance = (
                "Write a detailed summary with multiple paragraphs. Cover the main product families, categories, and any supported counts. "
                "Stay evidence-backed, but include enough context that the user can understand the full breadth of the portfolio."
            )
            depth_style = "detailed"
            depth_template = (
                "Template:\n"
                "1. Direct answer plus a concise caveat if the exact count is unknown.\n"
                "2. A grouped breakdown of product families and representative examples.\n"
                "3. A short closing note that summarizes the breadth of the evidence.\n"
            )
        else:
            depth_guidance = (
                "Write a balanced response with a few paragraphs. Keep it direct, but include enough context to be useful. "
                "If the user asks for breadth or detail, expand into grouped product families rather than a terse summary."
            )
            depth_style = "balanced"
            depth_template = (
                "Template:\n"
                "1. Answer the question directly.\n"
                "2. Add 1-2 supporting paragraphs with grouped evidence.\n"
                "3. End with a short closing sentence about confidence or ambiguity.\n"
            )

        product_titles = []
        for finding in (*evidence.memory_findings[:6], *evidence.web_findings[:3], *evidence.doc_findings[:3]):
            title = (finding.title or "").strip()
            if title and title not in product_titles:
                product_titles.append(title)
        product_summary = "; ".join(product_titles[:6]) if product_titles else "No specific product names were isolated from the evidence."
        ...
        answer = await self.complete_text(
            user_content=(
                ...
                "Speak like BLAIQ talking directly to the user, not like a report writer.\n\n"
                "Rules:\n"
                "- Answer the user's question directly using only the evidence provided below.\n"
                "- Match the chosen response depth instead of using a one-size-fits-all summary.\n"
                f"- Response depth selected by the user: {depth_value or 'not specified'} ({depth_style}).\n"
                f"- Depth guidance: {depth_guidance}\n"
                f"{depth_template}"
                "- Do not sound like a report unless the selected depth explicitly asks for a full technical breakdown.\n"
                ...
                "- If the evidence is insufficient for an exact answer, say so plainly in natural language.\n"
                ...
                "- For count questions, say whether the exact count is known; if it is not known, say that clearly and briefly.\n"
                "- If the evidence mentions product names or families, it is okay to name a few representative examples.\n\n"
                "USER QUESTION:\n"
                f"{query}\n\n"
                "EVIDENCE SNAPSHOT:\n"
                f"- Sources reviewed: {source_count}\n"
                f"- Representative product names or families: {product_summary}\n\n"
                f"- Selected response depth: {depth_value or 'not specified'}\n"
                "Now answer the user."
            ),
            extra_context={
                "query": query,
