...
class StrategicRoute(BaseModel):
    """Structured planner output. Replaces scattered heuristics with a single
    typed routing decision emitted by the ReAct planner layer.

    The compat export layer converts this into an existing ``StrategicDraft``
    so downstream engine code is unaffected until Phase 2 lands.
    """

    route: str = Field(
        description="One of: 'conversational', 'direct_answer', 'artifact'",
    )
    artifact_family: ArtifactFamily = ArtifactFamily.custom
    workflow_mode: WorkflowMode = WorkflowMode.hybrid
    analysis_mode: AnalysisMode = AnalysisMode.standard
    required_capabilities: list[str] = Field(default_factory=list)
    node_assignments: dict[str, str] = Field(
        default_factory=dict,
        description="node_id -> agent_id",
    )
    required_tools_per_node: dict[str, list[str]] = Field(
        default_factory=dict,
        description="node_id -> [tool_id, ...]",
    )
    missing_requirements: list[str] = Field(default_factory=list)
    reasoning_summary: str = ""

    def to_strategic_draft_kwargs(self) -> dict:
        """Export fields compatible with ``StrategicDraft`` constructor.

        Callers merge this with the rest of their draft kwargs so the compat
        export layer can inject the typed route without a full rewrite.
        """
        return {
            ...
            "required_tools_per_node": self.required_tools_per_node,
            "missing_requirements": self.missing_requirements,
            "topology_reason": self.reasoning_summary,
        }
...
def _agent_routing_card(agent: "LiveAgentProfile") -> dict:
    """Compact routing card — all fields a planner needs, no noise."""
    return {
        "name": agent.name,
        ...
        "planner_roles": list(agent.planner_roles),
        "artifact_affinities": list(agent.artifact_affinities),
...
from agentscope_blaiq.app.factory.spawner import (
...
class StrategicAgent(BaseAgent):
    def __init__(
        ...
        **kwargs,
        ...
        default_prompt = (
            "You are the Lead Strategist and Orchestrator at BLAIQ. Your core responsibility is to route user requests "
            "to the correct workflow and enforce strict role boundaries.\n\n"
            "ORCHESTRATION POLICIES:\n"
            "1. HIRE SPECIALISTS: If a task requires a specialized domain (e.g. Mailchimp Audit, SEO Research, API Design, Poetry), "
            "   ALWAYS check for existing experts using 'list_available_specialists'. If found, use 'hire_specialist'. "
            "   If not found, use 'spawn_specialist_agent' with a custom blueprint.\n"
            "2. RESEARCH-FIRST: If information is missing, ALWAYS attempt to resolve it via research nodes BEFORE "
            "   resorting to HITL.\n"
            "3. CHOOSE TOPOLOGY: Choose a template (visual_artifact_v1, text_artifact_v1, research_v1).\n"
            "4. STAGED is mandatory for Pitch Decks, Posters, Landing Pages, and Reports.\n"
            "5. DAG DESIGN: Assign nodes and define tool requirements for each stage.\n"
            "Your goal is to build the optimal DAG. Trigger specialists early to ensure high-fidelity inputs for the final artifact."
        )
        super().__init__(
            name=name,
            role=role,
            sys_prompt=sys_prompt or default_prompt,
            **kwargs,
        )
        self.catalog_provider = catalog_provider or (lambda: [])
        self.status_messages = [
            "Initializing BLAIQ intelligence grid...",
            "Analyzing requirement topology...",
            "Mapping execution strategy across live agents...",
        ]

    def build_toolkit(self) -> Toolkit:
        toolkit = Toolkit()
        self.register_tool(toolkit, tool_id="spawn_specialist_agent", fn=spawn_specialist_agent, description="Dynamically spawn a specialist worker agent based on a JSON blueprint.")
        self.register_tool(toolkit, tool_id="list_available_specialists", fn=list_available_specialists, description="List all available specialist blueprints in the factory library.")
        self.register_tool(toolkit, tool_id="hire_specialist", fn=hire_specialist, description="Hire an existing specialist by name to perform a task.")
        ...
        # Phase 6: narrow agent-profile lookup tools — avoid full catalog dumps
        self.register_tool(toolkit, tool_id="get_agent_profile", fn=self._tool_get_agent_profile, description="Return full profile for a single agent by name or profile_id.")
        self.register_tool(toolkit, tool_id="find_agents_by_capability", fn=self._tool_find_agents_by_capability, description="Find all agents that have a specific capability (e.g. 'web_research').")
        self.register_tool(toolkit, tool_id="find_agents_by_skill", fn=self._tool_find_agents_by_skill, description="Find all agents that have a specific skill (e.g. 'evidence_synthesis').")
        self.register_tool(toolkit, tool_id="find_agents_by_tool", fn=self._tool_find_agents_by_tool, description="Find all agents that carry a specific tool (e.g. 'hivemind_recall').")
        self.register_tool(toolkit, tool_id="find_agents_for_role", fn=self._tool_find_agents_for_role, description="Find and rank agents suitable for a task role, optionally filtered by artifact family.")
        return toolkit
    ...
    # ── Phase 6: narrow agent-profile lookup tools ────────────────────────────

    def _tool_get_agent_profile(self, agent_id: str | None = None):
        """Return full profile for a single agent by name or profile_id."""
        if not agent_id:
            return self.tool_response({"error": "agent_id required"})
        match = next(
            (a for a in self.catalog_provider()
             if a.name == agent_id or a.profile_id == agent_id),
            None,
        )
        if match is None:
            return self.tool_response({"error": f"No agent found with id '{agent_id}'"})
        return self.tool_response(_agent_routing_card(match))

    def _tool_find_agents_by_capability(self, capability: str | None = None):
        """Find agents that have a specific capability name."""
        if not capability:
            return self.tool_response({"error": "capability required"})
        cap_lower = capability.lower()
        matches = [
            _agent_routing_card(a)
            for a in self.catalog_provider()
            if any(cap_lower in c.name.lower() for c in a.capabilities)
        ]
        return self.tool_response({"capability": capability, "matches": matches, "count": len(matches)})

    def _tool_find_agents_by_skill(self, skill: str | None = None):
        """Find agents that have a specific skill name."""
        if not skill:
            return self.tool_response({"error": "skill required"})
        ...
        return self.tool_response({"skill": skill, "matches": matches, "count": len(matches)})

    def _tool_find_agents_by_tool(self, tool_id: str | None = None):
        """Find agents that carry a specific tool."""
        if not tool_id:
            return self.tool_response({"error": "tool_id required"})
        ...
        return self.tool_response({"tool_id": tool_id, "matches": matches, "count": len(matches)})

    def _tool_find_agents_for_role(
        self,
        role: str | None = None,
        artifact_family: str | None = None,
    ):
        """Find and rank agents suitable for a task role.

        Scores by: role match (3pt) + planner_role match (2pt) +
        artifact_family affinity (1pt) + ready status (1pt).
        """
        if not role:
            return self.tool_response({"error": "role required"})
        role_lower = role.lower()
        family_lower = (artifact_family or "").lower()
        ...
        for a in self.catalog_provider():
            ...
            if a.role.lower() == role_lower:
                score += 3
            if role_lower in [r.lower() for r in a.planner_roles]:
                score += 2
            if family_lower and any(
                family_lower in cap_fam.lower()
                for cap in a.capabilities
                for cap_fam in cap.supported_artifact_families
            ):
                score += 1
            if a.status.value == "ready":
                score += 1
            if score > 0:
                scored.append((score, _agent_routing_card(a)))

        scored.sort(key=lambda x: -x[0])
        results = [card for _, card in scored]
    ...
    async def _build_strategy_draft(
        ...
        *,
        ...
        await self.log_user("Understanding your request and selecting the workflow...")
        ...
        await self.log_user("Choosing the best agents and execution order...")
        if finance_mode:
            summary = (
        ...
        await self.log_user("Finalizing the execution plan...")
        await self.log(summary, kind="status", visibility="log")
        return draft
    ...
    async def _plan_with_react(
        self,
        request: SubmitWorkflowRequest,
        agent_catalog: list[LiveAgentProfile],
    ) -> StrategicRoute | None:
        """Tool-driven planner layer. Consolidates route, family, topology, and
        node assignments into a single typed ``StrategicRoute``.

        Runs the existing tool methods in sequenced order (classify → requirements
        → topology → task graph) and packages the result. When a ReAct LLM loop
        replaces this, the output contract stays the same — callers never change.

        Returns ``None`` on any exception so ``build_plan`` can fall back to the
        heuristic path cleanly.
        """
        try:
            # Step 1: Route classification (LLM-first, already implemented)
            await self.log_user("Classifying request intent and routing path...")
            route_str = await self.classify_route_llm(request)

            if route_str in {"conversational", "direct_answer"}:
                return StrategicRoute(
                    route=route_str,
                    artifact_family=ArtifactFamily.custom,
                    workflow_mode=WorkflowMode.sequential,
                    analysis_mode=request.analysis_mode,
                    reasoning_summary=f"Planner route: {route_str}.",
                )

            # Step 2: Artifact family (LLM-first with heuristic fallback)
            await self.log_user("Determining artifact family and blueprint requirements...")
            family = await self._classify_artifact_family_llm(
                request,
                self.classify_artifact_family(request),
            )

            # Step 3: Topology (LLM-first with heuristic fallback)
            await self.log_user("Selecting optimal workflow topology and orchestration mode...")
            mode = await self.choose_topology(request, agent_catalog)

            # Step 4: Requirements + task graph for stable node assignments
            await self.log_user("Composing task graph and assigning specialized agents...")
            checklist = self.compute_missing_requirements(
                self.derive_artifact_requirements(family, request)
            )
            task_graph = (
                self.compose_text_task_graph(family, checklist, agent_catalog)
                if family in TEXT_FAMILIES
                else self.compose_task_graph(family, checklist, agent_catalog)
            )
            ...
            return StrategicRoute(
                ...
                required_tools_per_node=self._required_tools_per_node(
                    task_graph, agent_catalog
                ),
                missing_requirements=list(checklist.missing_required_ids),
                reasoning_summary=(
                    f"Planner: {family.value} artifact, "
                    f"{mode.value} topology, "
                    f"{len(task_graph.nodes)} nodes."
                ),
            )
        except Exception:
            return None
    ...
    async def build_plan(
        ...
        await self.log_user("Analyzing your request and mapping the execution strategy...")
        catalog = agent_catalog if agent_catalog is not None else self.catalog_provider()

        # Start a fresh planner notebook for this planning session.
        self.create_notebook()


        # ReAct planner layer: produces StrategicRoute or None on failure.
        # Downstream code reads from react_route when available, falls back to
        # the existing heuristic calls so nothing breaks during the transition.
        react_route = await self._plan_with_react(request, catalog)

        route = react_route.route if react_route else await self.classify_route_llm(request)
        ...
        # Compat export layer: prefer react_route fields; fall back to individual
        # LLM calls so the existing heuristic path is untouched when react_route=None.
        if react_route is not None and react_route.artifact_family != ArtifactFamily.custom:
            artifact_family = react_route.artifact_family
        else:
            artifact_family = await self._classify_artifact_family_llm(
                request, self.classify_artifact_family(request),
            )
        analysis_mode = react_route.analysis_mode if react_route else request.analysis_mode
        ...
        # Prefer topology from react_route; fall back to LLM/heuristic call.
        if react_route is not None:
            mode = react_route.workflow_mode
        else:
        ...
        # Derive assignments directly from the task graph so they always match
        # the actual nodes (text_buddy for email, vangogh for visual, etc.).
        assignments = [
            AgentTaskAssignment(
                task_id=node.node_id,
                agent_name=node.assigned_to or "",
                role=node.task_role.value,
                reason=f"Assigned from task graph for {artifact_family.value}.",
                required_capabilities=list(node.required_capabilities),
                task_role=node.task_role.value,
        ...
        # Export notebook snapshot. When the ReAct LLM loop hasn't populated
        # current_plan yet (Python method path), synthesise snapshot from the
        # computed plan data so callers always get a non-null snapshot.
        self.revise_notebook()
        notebook_snapshot = self.export_notebook_snapshot()
        if notebook_snapshot is None:
            ...
            notebook_snapshot = {
                ...
                "agent": self.name,
                "route": react_route.route if react_route else "artifact",
                "artifact_family": artifact_family.value,
                "workflow_mode": mode.value,
                "workflow_template_id": draft.workflow_template_id,
                "node_assignments": draft.node_assignments,
                "missing_requirements": list(requirements.missing_required_ids),
                "topology_reason": draft.topology_reason,
            }
        planner_snapshot_json = json.dumps(notebook_snapshot)
        ...
        return WorkflowPlan(
            ...
            planner_snapshot_json=planner_snapshot_json,
        )
