from __future__ import annotations

from pydantic import BaseModel, Field
from agentscope.tool import Toolkit

from agentscope_blaiq.contracts.artifact import TextArtifact, VisualArtifact
from agentscope_blaiq.contracts.evidence import EvidencePack
from agentscope_blaiq.runtime.agent_base import BaseAgent
from agentscope_blaiq.tools.validation import validate_visual_artifact


class GovernanceReport(BaseModel):
    approved: bool
    issues: list[str] = Field(default_factory=list)
    readiness_score: float = 0.0
    notes: list[str] = Field(default_factory=list)


class GovernanceAgent(BaseAgent):
    def __init__(self, **kwargs) -> None:
        super().__init__(
            name="GovernanceAgent",
            role="governance",
            sys_prompt=(
                "You are the governance agent. Approve only when the artifact is complete, evidence-linked, and ready to ship. "
                "Otherwise emit explicit revision feedback."
            ),
            **kwargs,
        )

    def build_toolkit(self) -> Toolkit:
        toolkit = Toolkit()
        self.register_tool(toolkit, tool_id="validate_visual_artifact", fn=self._validate_artifact_contract, description="Run the platform's deterministic validation rules against a visual artifact payload.")
        return toolkit

    def _validate_artifact_contract(self, artifact_payload: dict):
        artifact = VisualArtifact.model_validate(artifact_payload)
        return self.tool_response(validate_visual_artifact(artifact))

    @staticmethod
    def _fallback_report(*, validation: dict, evidence: EvidencePack, issues: list[str]) -> GovernanceReport:
        approved = not issues
        notes = ["Fallback governance path used after non-JSON model output."]
        if evidence.citations:
            notes.append(f"Evidence linked to {len(evidence.citations)} citations.")
        else:
            notes.append("Evidence coverage is incomplete.")
        return GovernanceReport(
            approved=approved,
            issues=issues,
            readiness_score=float(validation["readiness_score"]) if approved else max(0.0, float(validation["readiness_score"]) - 0.2),
            notes=notes,
        )

    async def review(self, artifact: VisualArtifact, evidence: EvidencePack) -> GovernanceReport:
        await self.log_user("Validating output against requirements...")
        await self.log(
            f"Starting governance review for \"{artifact.title}\". Checking {len(artifact.sections)} sections against evidence.",
            kind="status",
            visibility="log",
        )

        validation = validate_visual_artifact(artifact)
        issues = list(validation["issues"])
        await self.log_user(
            "Checking evidence coverage and contract compliance...",
            detail={"readiness_score": validation["readiness_score"], "issue_count": len(issues)},
        )

        await self.log(
            f"Deterministic checks complete. Readiness score: {validation['readiness_score']}, issues: {len(issues)}.",
            kind="review",
            detail={"readiness_score": validation["readiness_score"], "issues": issues},
        )

        if not evidence.citations:
            issues.append("citations_missing")
            await self.log("Warning: No citations found in the evidence pack.", kind="review")

        if not issues:
            await self.log(
                f"All checks passed. Approving artifact with readiness score {validation['readiness_score']}.",
                kind="decision",
            )
            return GovernanceReport(
                approved=True,
                issues=[],
                readiness_score=float(validation["readiness_score"]),
                notes=["Artifact validated against deterministic governance checks."],
            )

        await self.log_user("Final approval review in progress...")
        await self.log(f"Issues detected: {', '.join(issues)}. Running model-backed review for nuanced assessment.", kind="thought")
        try:
            model_report = await self.complete_json(
                GovernanceReport,
                user_content="Review the artifact for readiness and return explicit approval or revision feedback. Respond with valid JSON only.",
                extra_context={
                    "artifact_title": artifact.title,
                    "artifact_type": artifact.artifact_type,
                    "section_count": len(artifact.sections),
                    "evidence_refs": artifact.evidence_refs,
                    "evidence_count": len(evidence.citations),
                    "local_validation": validation,
                },
                temperature=0.0,
                max_tokens=300,
            )
        except Exception:
            model_report = self._fallback_report(validation=validation, evidence=evidence, issues=sorted(set(issues)))

        approved = model_report.approved and not issues
        final_report = GovernanceReport(
            approved=approved,
            issues=sorted(set([*issues, *model_report.issues])),
            readiness_score=max(
                float(model_report.readiness_score),
                float(validation["readiness_score"]),
            )
            if approved
            else max(0.0, min(float(model_report.readiness_score), float(validation["readiness_score"])) - 0.2),
            notes=[*model_report.notes, "Artifact validated against v1 governance checks."],
        )

        verdict = "Approved" if final_report.approved else "Revision required"
        await self.log(
            f"{verdict}. Final readiness: {final_report.readiness_score}.",
            kind="decision",
            detail={"approved": final_report.approved, "readiness_score": final_report.readiness_score},
        )
        return final_report

    async def review_text(self, artifact: TextArtifact, evidence: EvidencePack) -> GovernanceReport:
        """Review a text artifact for brand-voice compliance, evidence linkage, and completeness."""
        await self.log_user("Validating output against requirements...")
        await self.log(
            f"Starting text governance review for \"{artifact.title}\" ({artifact.family}).",
            kind="status",
            visibility="log",
        )

        issues: list[str] = []
        if not artifact.content or len(artifact.content.strip()) < 50:
            issues.append("content_too_short")
        if not artifact.brand_voice_applied:
            issues.append("brand_voice_not_applied")
        if not artifact.evidence_refs:
            issues.append("no_evidence_refs")
        if not evidence.citations:
            issues.append("citations_missing")

        content_length = len(artifact.content)
        readiness_score = 1.0
        if issues:
            readiness_score = max(0.3, 1.0 - 0.15 * len(issues))
        await self.log_user(
            "Checking evidence coverage and contract compliance...",
            detail={"readiness_score": readiness_score, "issue_count": len(issues)},
        )

        if not issues:
            await self.log(
                f"All text checks passed. Approving with readiness {readiness_score:.2f}.",
                kind="decision",
            )
            return GovernanceReport(
                approved=True,
                issues=[],
                readiness_score=readiness_score,
                notes=[f"Text artifact validated ({content_length} chars, {artifact.family} family)."],
            )

        await self.log_user("Final approval review in progress...")
        await self.log(f"Issues detected: {', '.join(issues)}. Running model-backed review.", kind="thought")
        try:
            model_report = await self.complete_json(
                GovernanceReport,
                user_content="Review this text artifact for completeness, brand voice compliance, and evidence backing. Respond with valid JSON only.",
                extra_context={
                    "artifact_title": artifact.title,
                    "artifact_family": artifact.family,
                    "content_length": content_length,
                    "evidence_refs_count": len(artifact.evidence_refs),
                    "brand_voice_applied": artifact.brand_voice_applied,
                    "detected_issues": issues,
                },
                temperature=0.0,
                max_tokens=300,
            )
        except Exception:
            model_report = GovernanceReport(
                approved=not issues,
                issues=issues,
                readiness_score=readiness_score,
                notes=["Fallback governance path used for text artifact."],
            )

        approved = model_report.approved and not issues
        final_report = GovernanceReport(
            approved=approved,
            issues=sorted(set([*issues, *model_report.issues])),
            readiness_score=readiness_score if approved else max(0.3, readiness_score - 0.2),
            notes=[*model_report.notes, f"Text artifact governance ({artifact.family})."],
        )

        verdict = "Approved" if final_report.approved else "Revision required"
        await self.log(
            f"{verdict}. Final readiness: {final_report.readiness_score:.2f}.",
            kind="decision",
            detail={"approved": final_report.approved, "readiness_score": final_report.readiness_score},
        )
        return final_report
