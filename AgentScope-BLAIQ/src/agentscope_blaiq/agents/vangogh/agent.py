...
class VangoghAgent(BaseAgent):
    ...
    CAPABILITIES: list[AgentCapability] = [
        AgentCapability(
            ...
            description="Shape presentation decks and long-form visuals.",
            supported_task_types=["design", "presentation"],
            supported_task_roles=["vangogh"],
            supported_artifact_families=_VISUAL_FAMILIES,
        ),
        AgentCapability(
            name="html_css_composition",
            description="Produce HTML/CSS artifact previews.",
            supported_task_types=["artifact", "preview"],
            supported_task_roles=["vangogh"],
            supported_artifact_families=_VISUAL_FAMILIES,
        ),
    ]
...
5. Return ONLY the HTML fragment — no markdown fences, no comments"""
    ...
    async def reply(self, msg: Msg | str, *, extra_context: dict[str, Any] | None = None) -> Msg:
        """Process an ArtifactBrief directly and return a VisualArtifact."""
        context = extra_context or {}
        evidence = context.get("evidence_pack")
        brand_dna = context.get("brand_dna")
        user_query = msg.content if isinstance(msg, Msg) else str(msg)

        # ArtifactBrief expected in context or as a dict payload
        brief_data = context.get("artifact_brief") or context.get("content_brief")
        
        # If we have an ArtifactBrief, use the new path
        if brief_data:
            try:
                # 1. Transform ArtifactBrief component into PlannedContentBrief
                # (Vangogh's generator currently depends on PlannedContentBrief)
                if isinstance(brief_data, ArtifactBrief):
                    brief = self._artifact_brief_to_planned_brief(brief_data)
                elif isinstance(brief_data, dict) and "sections" in brief_data:
                    # Validated via ArtifactBrief model first
                    brief_obj = ArtifactBrief.model_validate(brief_data)
                    brief = self._artifact_brief_to_planned_brief(brief_obj)
                else:
                    # Fallback to legacy parsing if it looks like the old dict
                    brief = self._parse_content_brief(brief_data)
                
                # 2. Invoke generation
                artifact = await self.generate(
                    user_query=user_query,
                    evidence=evidence,
                    content_brief=brief.model_dump() if brief else None,
                    brand_dna=brand_dna,
                )
                ...
                logger.error(f"Vangogh ArtifactBrief processing failed: {e}")
                # Fallthrough to standard reply or error handle
        
        return await super().reply(msg, extra_context=extra_context)

    def _artifact_brief_to_planned_brief(self, brief: ArtifactBrief) -> PlannedContentBrief:
        """Convert canonical ArtifactBrief to Vangogh's internal PlannedContentBrief."""
        sections = []
        for s in brief.sections:
            sections.append(
                PlannedSection(
                    ...
                    headline=s.title, # Fallback headline
                    bullets=s.key_points,
                    evidence_refs=s.evidence_refs,
                    visual_intent=s.visual_hint or "",
                    acceptance_checks=s.constraints,
                )
            )
        
        return PlannedContentBrief(
            title=brief.title,
            family=brief.artifact_family.value,
            narrative=brief.core_narrative,
            audience=brief.target_audience,
            visual_direction=brief.style_preference or "executive",
