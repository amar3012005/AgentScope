...
def test_end_to_end_hybrid_workflow_with_upload(monkeypatch, tmp_path):
    ...
    async def fake_plan_content(*, user_query, evidence_summary, artifact_spec, requirements, hitl_answers=None, evidence_pack=None):
        class _Brief:
    ...
    async def fake_plan_slides(*, user_query, artifact_family, evidence_pack=None, hitl_answers=None, brand_dna=None, tenant_id="default", required_sections=None):
        class _Slides:
            def model_dump(self_inner):
                return {"slides": [], "title": user_query, "brand": "default", "layout": "slides"}

        return _Slides()

    def _make_fake_artifact(evidence):
    ...
    async def fake_generate(user_query, evidence, content_brief=None, on_section_ready=None, brand_dna=None):
        return _make_fake_artifact(evidence)

    async def fake_generate_from_slides(*, slides_data, user_query, evidence, brand_dna=None, artifact_family="custom", tenant_id="default", on_progress=None):
        return _make_fake_artifact(evidence)
    ...
    monkeypatch.setattr(main.registry.deep_research, "gather", fake_gather)
    monkeypatch.setattr(main.registry.content_director, "plan_content", fake_plan_content)
    monkeypatch.setattr(main.registry.content_director, "plan_slides", fake_plan_slides)
    monkeypatch.setattr(main.registry.vangogh, "generate", fake_generate)
    monkeypatch.setattr(main.registry.vangogh, "generate_from_slides", fake_generate_from_slides)
...
def test_resume_after_hitl_does_not_reblock_when_only_some_answers_are_provided(monkeypatch, tmp_path):
    db_path = tmp_path / "agentscope-e2e-hitl-partial-resume.db"
    upload_dir = tmp_path / "uploads"
    artifact_dir = tmp_path / "artifacts"
    log_dir = tmp_path / "logs"
            ...
            requirements_checklist=RequirementsChecklist(
                items=[
                    ...
                    RequirementItem(
                        ...
                        text="Provide the Problem for the custom artifact.",
                        category="section",
                        source="artifact_family",
                        priority=2,
                        must_have=True,
                ...
                missing_required_ids=["section:hero", "section:problem", "field:must_have_sections"],
            ),
            tasks=[
                AgentRunPayload(agent_type=AgentType.research, purpose="Research web", branch_key="web"),
                AgentRunPayload(agent_type=AgentType.research, purpose="Research docs", branch_key="docs"),
    ...
    async def fake_generate(user_query, evidence, content_brief=None, on_section_ready=None, brand_dna=None):
        return VisualArtifact(
            artifact_id="artifact-hitl-resume-2",
            title="Resume Without Replay",
            sections=[
                ArtifactSection(
                    section_id="hero",
    ...
    thread_id = "thread-e2e-hitl-partial-resume-001"
    tenant_id = "tenant-e2e"
    ...
    with TestClient(main.app) as client:
        ...
        with client.stream(
            ...
            json={
                ...
                "workflow_mode": "hybrid",
                "tenant_id": tenant_id,
                "session_id": "session-e2e-hitl-partial-resume-001",
                "thread_id": thread_id,
                "artifact_type": "visual_html",
                "source_scope": "web_and_docs",
            },
        ...
        assert resume_types[0] == "workflow_resumed"
        assert "resume_accepted" in resume_types
        assert "workflow_blocked" not in resume_types
        assert "content_director_started" in resume_types
        assert "workflow_complete" in resume_types
