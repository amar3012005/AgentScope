...
function agentLabel(name) {
  ...
  if (k.includes('text_buddy') || k.includes('textbuddy')) return 'Text Writer';
...
function formatStrategyMessage(plan) {
  ...
  const analysisMode = sentenceCase(plan.analysis_mode || 'standard');
  ...
  return `${lead}\n\nExecution mode: ${mode}. Analysis mode: ${analysisMode}. Tasks: ${plan.tasks.length}.${noteBlock}\n${steps}${fanIn}`;
}
...
function formatAgentLiveMessage(event, mapped) {
  ...
  switch (event.type) {
    ...
    case 'workflow_resumed':
      // Skip interim resume message for cleaner UX
      return null;
      // return {
      //   agent: 'BLAIQ-CORE',
    ...
    case 'planning_started':
      // Skip interim planning message — planning_complete will show the summary
      return null;
    case 'requirements_check_started':
      // Skip interim requirements check message — completion event has the summary
      return null;
    ...
    case 'artifact_family_selected':
      // Skip this verbose interim message for cleaner UX
      return null;
      // return {
      //   agent: 'BLAIQ-CORE',
...
function buildArtifactPreview(task) {
  ...
  // Pick up theme CSS emitted with the first section
  const themeCss = sections[0]?.section_data?.theme_css || css || '';
  const rawHtml = `<!doctype html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap" rel="stylesheet"><title>${title}</title><style>${themeCss}</style></head><body>${fragments}</body></html>`;
  return {
    rawHtml,
...
function makeSteps(mode) {
  ...
  if (mode === 'parallel' || mode === 'hybrid' || mode === 'deep_research' || mode === 'finance') {
...
function mapEvent(event) {
  ...
  switch (t) {
    ...
    case 'parallel_branch_started':
      ...
      if (event.phase === 'text_buddy') {
        return { stepId: 'content_director', status: 'active', detail: 'Composing text in brand voice', agent };
      }
    ...
    case 'parallel_branch_completed':
      ...
      if (event.phase === 'text_buddy') {
        return { stepId: 'content_director', status: 'done', detail: 'Text composition complete', agent };
      }
    ...
    case 'agent_log': {
      ...
      else if (phase === 'content_director' || phase === 'text_buddy') sid = 'content_director';
...
export function BlaiqWorkspaceProvider({ children }) {
  ...
  const [sessionId, setSessionId] = useState(() => {
    try {
      const saved = window.localStorage.getItem('blaiq.sessionId');
  ...
  const [sessions, setSessions] = useState(() => {
    try {
      const saved = window.localStorage.getItem('blaiq.sessions');
  ...
  React.useEffect(() => {
    try {
      window.localStorage.setItem('blaiq.sessions', JSON.stringify(sessions));
    } catch {}
  }, [sessions]);

  // Persist current sessionId to localStorage
  React.useEffect(() => {
    try {
      window.localStorage.setItem('blaiq.sessionId', sessionId);
      sessionIdRef.current = sessionId;
    } catch {}
  }, [sessionId]);

  // Persist tasks for current session whenever they change
  React.useEffect(() => {
    try {
      if (tasks.length > 0) {
        window.localStorage.setItem(`blaiq.session.${sessionId}.tasks`, JSON.stringify(tasks));
      }
    } catch {}
  }, [tasks, sessionId]);

  // Restore tasks for current session on mount
  React.useEffect(() => {
    try {
      const saved = window.localStorage.getItem(`blaiq.session.${sessionId}.tasks`);
  ...
  React.useEffect(() => {
    const sessionTaskIds = new Set(tasks.filter((task) => task.sessionId === sessionId).map((task) => task.id));
    if (activeTaskId && !sessionTaskIds.has(activeTaskId)) {
      const latestTask = tasks
        .filter((task) => task.sessionId === sessionId)
        .sort((left, right) => new Date(left.createdAt).getTime() - new Date(right.createdAt).getTime())
        .at(-1);
      setActiveTaskId(latestTask?.id || null);
    }
  }, [activeTaskId, sessionId, tasks]);

  function ensureSessionRecord(targetSessionId) {
    if (!targetSessionId) return targetSessionId;
    setSessions((prev) => {
      if (prev.some((session) => session.id === targetSessionId)) {
        return prev;
      }
      return [
        {
  ...
  function switchSession(newSessionId) {
    ensureSessionRecord(newSessionId);
    // Save current session tasks before switching
    ...
    // Restore tasks for the new session
    try {
      const saved = window.localStorage.getItem(`blaiq.session.${newSessionId}.tasks`);
  ...
  function updateSessionTitle(sessionIdToUpdate, title) {
    setSessions((prev) => prev.map((s) =>
      s.id === sessionIdToUpdate ? { ...s, title, lastUsedAt: new Date().toISOString() } : s
    ));
  }
  ...
  function updateSessionMemoryChain(sessionIdToUpdate, memoryChainId) {
    setSessions((prev) => prev.map((s) =>
      s.id === sessionIdToUpdate ? { ...s, memoryChainId, lastUsedAt: new Date().toISOString() } : s
    ));
  }
  ...
  const agentPlan = useMemo(() => {
    if (!activeTask?.routingDecision?.plan) return null;
    const plan = activeTask.routingDecision.plan;
    const tasks = (plan.tasks || []).map((task, idx) => ({
      id: task.id || `task-${idx}`,
      title: task.purpose || task.agent_type || `Task ${idx + 1}`,
  ...
  const renderState = useMemo(() => {
    ...
    const contentDirectorStep = (activeTask.steps || []).find((step) => step.id === 'content_director');
    return {
      ...
      isContentDirectorActive: contentDirectorStep?.status === 'active',
    };
  }, [activeTask]);
  ...
  function createTask(userQuery, workflowMode, sessionId) {
    const id = generateId();
    const task = {
      id,
      query: userQuery,
    ...
    // Auto-set session title from first message (if still "New Chat")
    setSessions((prev) => prev.map((s) => {
      if (s.id === sessionId) {
        const titleBase = userQuery.length > 50 ? userQuery.slice(0, 50) + '...' : userQuery;
        const title = `${titleBase} · ${sessionId.slice(0, 8)}`;
        return { ...s, title, lastUsedAt: new Date().toISOString() };
      }
      return s;
    }));
  ...
  function handleEvent(taskId, event) {
    ...
    setTasks((prev) => prev.map((t) => {
      ...
      // Thread ID + memory chain
      if (event.thread_id) updated.threadId = event.thread_id;
      if (event.data?.memory_chain_id) {
        updated.memoryChainId = event.data.memory_chain_id;
        // Update session memory chain immediately to avoid race condition
        if (updated.sessionId) {
          updateSessionMemoryChain(updated.sessionId, event.data.memory_chain_id);
        }
      }
  ...
  const submit = useCallback(async (userQuery, workflowMode, analysisMode = 'standard') => {
    if (!userQuery?.trim() || isSubmitting) return;
    setIsSubmitting(true);

    const mode = workflowMode || 'hybrid';

    // For multi-turn: always reuse the current chat session sid. New chat is created explicitly.
    const currentSession = sessions.find((s) => s.id === sessionId);
    const existingChainId = currentSession?.memoryChainId || null;
    const effectiveSessionId = currentSession?.id || ensureSessionRecord(sessionId);

    const task = createTask(userQuery.trim(), mode, effectiveSessionId);
