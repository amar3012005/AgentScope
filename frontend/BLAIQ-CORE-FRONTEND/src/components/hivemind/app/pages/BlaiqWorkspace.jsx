          ...
          <div className={`grid min-h-0 ${rightRailOpen ? 'grid-cols-[minmax(0,1fr)_420px]' : 'grid-cols-[minmax(0,1fr)]'} overflow-hidden`}>
            <section className={`grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] ${rightRailOpen ? 'border-r border-[#e3e0db]' : ''}`}>
              ...
              <div className="border-t border-[#e3e0db] bg-white px-6 py-5">
                <AnimatePresence>
                  {hitl.open ? (
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} className="mb-4 border border-[#117dff]/15 bg-[#f8fbff] p-4">
                      <div className="mb-3 flex items-center justify-between">
                        <div>
                          <div className="text-[11px] font-mono uppercase tracking-[0.12em] text-[#a3a3a3]">Clarification needed</div>
                          <div className="font-['Space_Grotesk'] text-base font-semibold text-[#0a0a0a]">Answer inline and continue rendering</div>
                        </div>
                      ...
                      <div className="space-y-3">
                        {hitl.questions.map((question, index) => (
                          <div key={`${question}-${index}`} className="border-t border-[#dbe8fb] pt-3 first:border-t-0 first:pt-0">
                            <div className="mb-1 text-[10px] font-mono uppercase tracking-[0.12em] text-[#a3a3a3]">Question {index + 1}</div>
                            <div className="mb-2 text-sm font-medium text-[#0a0a0a]">{question}</div>
                            <textarea
                              value={hitl.answers[`q${index + 1}`] || ''}
                              onChange={(event) => setHitl((current) => ({
                                ...current,
                                answers: { ...current.answers, [`q${index + 1}`]: event.target.value },
                              }))}
                              rows={2}
                              className="mb-2 w-full resize-none rounded-sm border border-[#e3e0db] bg-white px-3 py-2 text-sm text-[#0a0a0a] outline-none focus:border-[#117dff]/40"
                            />
                            <div className="flex flex-wrap gap-2">
                              {quickChips(question).map((chip) => (
                                <button
                                  key={chip}
                                  type="button"
                                  onClick={() => setHitl((current) => ({
                                    ...current,
                                    answers: { ...current.answers, [`q${index + 1}`]: chip },
                                  }))}
                                  className="rounded-sm border border-[#e3e0db] bg-white px-3 py-1.5 text-[11px] text-[#525252] transition-colors hover:border-[#117dff]/20 hover:text-[#117dff]"
                                >
                                  {chip}
                                </button>
                              ))}
                      ...
                      <div className="mt-4 flex justify-end">
                        <button type="button" disabled={isResuming} onClick={handleResume} className="rounded-md bg-[#111827] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#1f2937] disabled:opacity-50">
                          {isResuming ? 'Resuming...' : 'Continue rendering'}
                        </button>
                      </div>
                    </motion.div>
                  ) : null}
