...
import { useBlaiqWorkspace, normalizePreviewHtml } from '../shared/blaiq-workspace-context';
    ...
    <div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_360px] gap-6 overflow-hidden p-5 md:p-7 max-xl:grid-cols-[minmax(0,1fr)]">
      <section className="min-h-0 rounded-[32px] border border-[rgba(0,0,0,0.06)] bg-[#faf9f4] p-4 shadow-[0_22px_50px_rgba(0,0,0,0.06)]">
        ...
        <div className="h-[calc(100%-68px)] overflow-hidden rounded-[26px] border border-[#e3e0db] bg-white">
          {previewHtml ? (
            <iframe title="Artifact preview" srcDoc={normalizePreviewHtml(previewHtml)} className="h-full w-full bg-white" />
          ) : (
        ...
        <div className="rounded-[30px] border border-[rgba(0,0,0,0.06)] bg-[#faf9f4] p-5 shadow-[0_18px_44px_rgba(0,0,0,0.05)]">
          ...
          {governance ? (
            <div className="space-y-3">
              <div className={`rounded-[22px] px-3 py-2 text-sm font-medium ${governance.validation_passed ? 'bg-emerald-50 text-emerald-700' : 'bg-[rgba(255,92,75,0.12)] text-[#ff5c4b]'}`}>
                {governance.validation_passed ? 'Validation passed' : 'Review required'}
              </div>
              {(governance.policy_checks || []).map((check) => (
                <div key={`${check.rule}-${check.detail}`} className="rounded-[22px] bg-white p-3">
                  <div className="text-sm font-medium text-[#111827]">{check.rule}</div>
                  <div className="mt-1 text-xs text-[#6b7280]">{check.detail}</div>
                </div>
              ))}
            </div>
