...
class FakeHivemindClient(HivemindMCPClient):
    ...
    async def recall(self, *, query: str, limit: int = 20, mode: str = "insight"):
        ...
        if "tokyo trip" in query_key:
            return {
                "memories": [
                    {
                        "memory_id": "mem-tokyo",
                        "title": "Tokyo trip planning",
                        "content": "You are planning a trip to Tokyo next month.",
                        "score": 0.86,
                    }
                ]
            }
...
@pytest.mark.asyncio
async def test_personal_project_queries_use_single_memory_pass():
    hivemind = FakeHivemindClient()
    agent = FakeResearchAgent(hivemind=hivemind)
    ...
    evidence = await agent.gather(
        session=None,
        tenant_id="tenant-1",
        query="Create a professional pitch deck presentation about my tokyo trip",
        scope="web",
    )

    assert hivemind.recall_queries == ["Create a professional pitch deck presentation about my tokyo trip"]
...
async def test_personal_memory_query_uses_quick_recall_and_skips_web_without_freshness_need():
    ...
    assert hivemind.recall_modes == ["quick"]
    assert hivemind.recall_limits == [5]
...
@pytest.mark.asyncio
async def test_injection_text_is_ignored_for_project_queries():
    ...
    assert len(evidence.memory_findings) == 1
    assert evidence.memory_findings[0].source_ids[0] == "mem-projects"
    assert not any(f.source_ids[0].startswith("injection:") for f in evidence.memory_findings)


@pytest.mark.asyncio
async def test_research_agent_continues_when_one_recall_pass_fails():
    ...
    assert hivemind.recall_calls >= 2
    assert evidence.memory_findings
